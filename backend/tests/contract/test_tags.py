...
@pytest.mark.asyncio
async def test_create_duplicate_tag_rejected(client: AsyncClient, auth_headers: dict):
    await client.post("/api/v1/tags", json={"id": "", "name": "政治"}, headers=auth_headers)
    resp = await client.post("/api/v1/tags", json={"id": "", "name": "政治"}, headers=auth_headers)
    assert resp.status_code == 409


@pytest.mark.asyncio
async def test_tag_case_count(client: AsyncClient, auth_headers: dict):
    tag_resp = await client.post(
        "/api/v1/tags", json={"id": "", "name": "经济"}, headers=auth_headers
    )
